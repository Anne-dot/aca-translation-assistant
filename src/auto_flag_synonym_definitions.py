...
def find_definitions_in_synonyms():
    ...
    for item in candidates:
        ...
        if len(synonym_text) > 40 and any(
            word in synonym_text.lower()
            for word in ["of", "the", "a person", "someone", "something"]
        ):
            reason = f'Long explanatory phrase: "{synonym_text}"'
        elif any(
            pattern in synonym_text.lower()
            for pattern in [
                "a person who",
                "someone who",
                "the act of",
            ...
            synonym.strip().split()[0].endswith("ing") and len(synonym) > 30
