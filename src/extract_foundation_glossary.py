...
from utils import (
    read_csv_file, save_json_file, ensure_output_dir,
    clean_text, parse_list_from_text, detect_numbered_meanings, split_numbered_text
)
...
def parse_term_row(row):
    """
    Parse single CSV row into term data.

    Args:
        row: CSV row (list)

    Returns:
        dict with term data or None if should skip
    """
