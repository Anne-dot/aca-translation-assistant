...
def parse_term_row(row):
	...
	if has_numbered_meanings(definition):
		...
		# TODO: see if this could be done cleaner
		meanings = []
		for i in range(len(def_parts)):
			meaning = {
				"definition": (
					clean_text(def_parts[i]) if i < len(def_parts)
					else ""
				),
				"synonyms": (
					parse_list_from_text(syn_parts[i]) if i < len(syn_parts)
					else []
				),
				"usageExample": (
					clean_text(ex_parts[i]) if i < len(ex_parts)
					else ""
				)
			}
...
def extract_foundation_glossary():
	...
	print(f"-   Skipped: {skipped_markers} letter markers")
