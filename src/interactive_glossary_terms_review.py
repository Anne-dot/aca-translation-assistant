...
""")
    ...
    if issues:
        ...
        for issue in issues:
            print(f"\t- {get_issue_description_short(issue)}")
...
def handle_normalization_edit(issue):
    ...
    match issue["category"]:
        case "split_parentheses" | "split_multiple_comma" | "split_multiple_slash":
            ...
            return {"type": issue["category"], "data": terms}
        ...
        case "remove_asterisk":
            ...
            return {
                "type": issue["category"],
                "data": {"cleanTerm": clean, "note": note if note else None},
            }
...
def filter_terms_for_review(terms, review_mode):
    ...
    def not_waiting(t):
        return not t.get("waitingForUpdate", False)
...
""")
        ...
        display_complete_term_info(
            term, title=filter_type, index=i, total=len(terms_to_review)
        )
        ...
        ACTION_FUNCTIONS = {
            "a": accept_term,
            "w": mark_waiting_for_update,
            "f": flag_term_for_review,
            "m": merge_term_meanings,
            "e": edit_term_meanings,
            "d": edit_definition,
            "t": edit_term_fields,
            "n": edit_review_notes,
        }
