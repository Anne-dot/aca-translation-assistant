...
def detect_multiple_terms_comma(term_data):
    ...
    if len(parts) > 1:
        return {"category": "split_multiple_comma", "suggestion": parts}
...
def detect_multiple_terms_slash(term_data):
    ...
    parts = [p.strip() for p in term.split("/")]
    if len(parts) > 1:
        return {"category": "split_multiple_slash", "suggestion": parts}
...
def collect_all_issues(term_data):
    ...
    for check in [
        detect_parentheses_notation,
        detect_asterisk,
        detect_seealso_issues,
        detect_multiple_terms_comma,
        detect_multiple_terms_slash,
    ]:
